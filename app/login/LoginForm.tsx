...
const ADMIN_EMAIL = "sarah.russell@lumenalta.com";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({
          email: loginAsAdmin ? ADMIN_EMAIL : email,
          password,
        }),
      });
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => {
                ...
                if (e.target.checked) setEmail(ADMIN_EMAIL);
              }}
              className="rounded border-brand-light-gray-violet text-brand-accent focus:ring-brand-accent"
            />
            <label
              htmlFor="login-as-admin"
              className="text-sm text-brand-charcoal-violet"
            >
              Login as admin
            </label>
          </div>
